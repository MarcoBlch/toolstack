...
export default function ShippingClient({
  ...
  const result = useMemo(() => {
    ...
    return {
      ...
      isDomestic,
      tiers: [
        { name: 'Economy', days: '14-30 days', cost: economy },
        { name: 'Standard', days: '7-14 days', cost: standard },
        { name: 'Express', days: '3-5 days', cost: express },
      ],
  ...
  return (
    <ToolShell name="Shipping Calculator" icon="📦" currentPath="/shipping-calculator">
      <div style={{ background: '#FAFAF8', minHeight: '100vh', color: '#1C1B18', fontFamily: fb }}>
        <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 28px', maxWidth: 700, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 17, fontWeight: 700 }}>Shipping Calculator</span>
          </div>
        ...
        <section style={{ maxWidth: 700, margin: '0 auto', padding: '32px 28px 16px', textAlign: 'center' }}>
          <h1 style={{ fontSize: 'clamp(26px, 4vw, 38px)', fontWeight: 800, letterSpacing: '-1px', marginBottom: 8 }}>
            Shipping <span style={{ color: accent }}>Cost</span> Calculator
          </h1>
          <p style={{ fontSize: 14, color: '#6B6560', marginBottom: 24 }}>Estimate shipping costs by weight and dimensions. Compare carriers and delivery speeds.</p>
        </section>
        ...
        <section style={{ maxWidth: 700, margin: '0 auto', padding: '0 28px 24px' }}>
          <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #E8E4DB', padding: 28 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={labelStyle}>Package Weight</label>
        ...
        {result && (
          <>
            ...
            <section style={{ maxWidth: 700, margin: '0 auto', padding: '0 28px 16px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E4DB', padding: '22px 16px', textAlign: 'center' }}>
                  <div style={labelStyle}>Actual Weight</div>
                  <div style={{ fontSize: 22, fontFamily: fm, fontWeight: 700, color: '#1C1B18' }}>{fmt(result.actualWeightKg)}</div>
                  <div style={{ fontSize: 11, color: '#9A958A', marginTop: 2 }}>kg</div>
                </div>
                <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E4DB', padding: '22px 16px', textAlign: 'center' }}>
                  <div style={labelStyle}>Dimensional Weight</div>
                  <div style={{ fontSize: 22, fontFamily: fm, fontWeight: 700, color: '#1C1B18' }}>{fmt(result.dimWeight)}</div>
                  <div style={{ fontSize: 11, color: '#9A958A', marginTop: 2 }}>kg</div>
                </div>
                <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E4DB', padding: '22px 16px', textAlign: 'center' }}>
                  <div style={labelStyle}>Billable Weight</div>
                  <div style={{ fontSize: 22, fontFamily: fm, fontWeight: 700, color: accent }}>{fmt(result.billableWeight)}</div>
                  <div style={{ fontSize: 11, color: '#9A958A', marginTop: 2 }}>kg (max of both)</div>
                </div>
              </div>
            </section>
            ...
            <section style={{ maxWidth: 700, margin: '0 auto', padding: '0 28px 16px' }}>
              <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #E8E4DB', padding: 28 }}>
                <div style={{ ...labelStyle, marginBottom: 14 }}>Estimated Shipping Costs</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  {result.tiers.map((tier, i) => (
                    <div key={tier.name} style={{
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: i === 2 ? accent : '#1C1B18', marginBottom: 4 }}>{tier.name}</div>
            ...
            <section style={{ maxWidth: 700, margin: '0 auto', padding: '0 28px 24px' }}>
              <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #E8E4DB', padding: 28 }}>
                <div style={{ ...labelStyle, marginBottom: 14 }}>Carrier Comparison (Estimated)</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr>
                        <th style={{ textAlign: 'left', padding: '8px 10px', borderBottom: '1.5px solid #E8E4DB', color: '#9A958A', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.5px' }}>Carrier</th>
                        <th style={{ textAlign: 'center', padding: '8px 10px', borderBottom: '1.5px solid #E8E4DB', color: '#9A958A', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.5px' }}>Economy</th>
                        <th style={{ textAlign: 'center', padding: '8px 10px', borderBottom: '1.5px solid #E8E4DB', color: '#9A958A', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.5px' }}>Standard</th>
                        <th style={{ textAlign: 'center', padding: '8px 10px', borderBottom: '1.5px solid #E8E4DB', color: '#9A958A', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.5px' }}>Express</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.carriers.map(c => (
                        <tr key={c.name}>
                          ...
                          <td style={{ padding: '10px', borderBottom: '1px solid #F5F3EE', textAlign: 'center', fontFamily: fm, fontWeight: 600 }}>${fmt(c.economy)}</td>
                          <td style={{ padding: '10px', borderBottom: '1px solid #F5F3EE', textAlign: 'center', fontFamily: fm, fontWeight: 600 }}>${fmt(c.standard)}</td>
                          <td style={{ padding: '10px', borderBottom: '1px solid #F5F3EE', textAlign: 'center', fontFamily: fm, fontWeight: 600, color: accent }}>${fmt(c.express)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <section style={{ maxWidth: 540, margin: '0 auto', padding: '32px 28px', borderTop: '1px solid #E8E4DB' }}>
          ...
          <h3 style={{ fontSize: 15, fontWeight: 700, marginTop: 18, marginBottom: 6 }}>Domestic vs International Shipping</h3>
          <p style={{ fontSize: 13, color: '#6B6560', lineHeight: 1.8 }}>
