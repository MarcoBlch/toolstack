...
import { FALLBACK_RATES } from '@/lib/getRates'
...
const LABELS: Record<string, Record<Locale, string>> = {
  ...
  // Disclaimer (static fallback when no rateDate provided)
  disclaimer: {
    en: 'Exchange rates are approximate. For real-time rates, check your bank.',
    fr: 'Les taux de change sont approximatifs. Pour les taux en temps réel, consultez votre banque.',
    es: 'Los tipos de cambio son aproximados. Para tipos en tiempo real, consulte su banco.',
    pt: 'As taxas de câmbio são aproximadas. Para taxas em tempo real, consulte seu banco.',
    de: 'Wechselkurse sind Näherungswerte. Für Echtzeitkurse wenden Sie sich an Ihre Bank.',
  },
  // Rate date note prefix ("Rates as of {date}. Updated daily.")
  ratesAsOf: { en: 'Rates as of', fr: 'Taux au', es: 'Tasas al', pt: 'Taxas de', de: 'Kurse vom' },
  updatedDaily: { en: 'Updated daily.', fr: 'Mises à jour quotidiennement.', es: 'Actualizadas diariamente.', pt: 'Atualizadas diariamente.', de: 'Täglich aktualisiert.' },
  preciseRates: {
    en: 'For precise rates, check your bank.',
    fr: 'Pour des taux précis, consultez votre banque.',
    es: 'Para tasas precisas, consulte su banco.',
    pt: 'Para taxas precisas, consulte seu banco.',
    de: 'Für genaue Kurse wenden Sie sich an Ihre Bank.',
  },
...
export default function CurrencyClient({
  ...
  const RATES = ratesProp ?? FALLBACK_RATES
        ...
        {results && (
          <section style={{ maxWidth: 700, margin: '0 auto', padding: '0 28px 24px' }}>
            ...
            <div style={{
              ...
              {rateDate
                ? `${lt('ratesAsOf')} ${rateDate}. ${lt('updatedDaily')} ${lt('preciseRates')}`
                : lt('disclaimer')}
            </div>
          </section>
        )}
